...
export default class CompeticoesJ1 extends Vue {
  private all_competicoes: Array<any> = [];
  ...
  loadCompeticoes() {
    ...
    const competicoesRef = ref(db, 'versao1/competicao');

    onValue(competicoesRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        this.all_competicoes = Object.keys(data).map((key) => ({
          id: key,
          ...data[key],
        }));
        console.log('Competições loaded:', this.all_competicoes); // Debug log
      } else {
        console.log('No data found in Firebase'); // Debug log
      }
    }, (error) => {
      console.error('Error loading competições:', error); // Debug log
    });
  }
}
</script>
